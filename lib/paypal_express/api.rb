  ...
  module PaypalExpress #:nodoc:
    class PaymentPlugin < ::Killbill::Plugin::ActiveMerchant::PaymentPlugin
      ...
      def get_payment_info(kb_account_id, kb_payment_id, properties, context)
        ...
        if only_pending_transaction
          ...
          if cancel_pending_transaction(t_info_plugins.last).nil?
            logger.warning("Try to cancel pending transaction #{t_info_plugins.kb_transaction_payment_id} but failed")
            t_info_plugins
          else
            logger.info("Cancel pending transaction #{t_info_plugins.kb_transaction_payment_id}")
        ...
        if ::Killbill::Plugin::ActiveMerchant::Utils.normalized(properties_hash, :from_hpp)
          ...
          response = @response_model.create(:api_call                     => :build_form_descriptor,
                                            ...
                                            :message                      => { :payment_plugin_status => :PENDING,
                                                                               :token_expiration_period => token_expiration_period}.to_json)
      ...
      def token_expired(transaction_plugin_info)
        return false
        property_hash = properties_to_hash(transaction_plugin_info.properties)
        message_details = JSON.parse(::Killbill::Plugin::ActiveMerchant::Utils.normalized(property_hash, :message))
        return false if message_details.nil?
        expiration_period = (Killbill::Plugin::ActiveMerchant::Utils.normalized(message_details, :token_expiration_period) || THREE_HOURS_AGO).to_i
